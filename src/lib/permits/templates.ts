...
export function buildBusinessPermitHTML(p: PermitIssuanceLocal) {
...
<body>
  <div class="header">
    <div class="h1">${esc(p.barangayName)}, ${esc(p.municipalityCity)}, ${esc(p.province)}</div>
  ...
  <div class="sig">
    ...
    <div class="line">${esc(p.issuedByName)}<br/>Issuing Officer</div>
  </div>
