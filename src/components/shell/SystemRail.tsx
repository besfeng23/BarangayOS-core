...
export default function SystemRail({
  ...
  const userRole = getRolePill(user?.customClaims?.role);

  const raw = (settings?.barangayName ?? 'Barangay').trim();
  const withoutBarangay = raw.replace(/^barangay\s+/i, '').trim();
  const token =
    (withoutBarangay.length ? withoutBarangay : 'BARANGAY').toUpperCase();
        ...
        >
          <div className="h-10 w-10 rounded-xl border bg-background flex items-center justify-center">
            ...
            >
              ...
              />
              <path
                d="M14.6 14.6L15.7 15.7"
                stroke="currentColor"
                strokeWidth="1.6"
                strokeLinecap="round"
              />
              <path
                d="M15.7 8.3L14.6 9.4"
                stroke="currentColor"
                strokeWidth="1.6"
                strokeLinecap="round"
              />
              <path
                d="M9.4 14.6L8.3 15.7"
                stroke="currentColor"
                strokeWidth="1.6"
                strokeLinecap="round"
              />
          ...
          <div className="hidden sm:block leading-none min-w-0">
            <div className="text-foreground font-semibold truncate">
              {settings?.barangayName ?? 'BarangayOS'}
            </div>
